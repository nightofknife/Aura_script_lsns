...
from plans.resonance.src.actions.battle_dispatch_actions import (
    ...
    resonance_prepare_battle_formation,
...
class TestResonanceBattleDispatchActions(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        combat_task_file = Path("plans/resonance/tasks/auto_battle_combat.yaml")
    ...
    def test_ocr_normalization_ignores_action_summary_decorative_punctuation(self):
        expected = battle_dispatch_actions._normalize_text("特供·救世")
        self.assertEqual(battle_dispatch_actions._normalize_text("“特供?救世"), expected)
        self.assertEqual(battle_dispatch_actions._normalize_text("特供•救世"), expected)

    def test_global_supply_ocr_labels_use_only_distinctive_suffixes(self):
        self.assertEqual(battle_dispatch_actions._ACTION_SUMMARY_STAGE_OCR_TEXT["savior"], "救世")
        self.assertEqual(battle_dispatch_actions._ACTION_SUMMARY_STAGE_OCR_TEXT["standard"], "制式")
        self.assertEqual(battle_dispatch_actions._ACTION_SUMMARY_STAGE_OCR_TEXT["elegant"], "雅致")
    ...
    @patch("plans.resonance.src.actions.battle_dispatch_actions.time.sleep")
    def test_prepare_battle_formation_keeps_current_when_not_requested(self, sleep_mock):
        app = Mock()

        out = resonance_prepare_battle_formation(
    ...
    def test_prepare_battle_formation_rejects_invalid_slot(self):
        ...
        with self.assertRaises(BattleDispatchError) as cm:
            resonance_prepare_battle_formation(formation_index=5, app=app)

        self.assertEqual(cm.exception.code, "invalid_formation_index")
        app.click.assert_not_called()

    def test_combat_task_uses_single_formation_action(self):
        steps = self.combat_task_data["auto_battle_combat"]["steps"]
        ...
        self.assertEqual(
            ...
            {
                "action": "resonance.prepare_battle_formation",
                "params": {
    ...
    def test_combat_inputs_are_forwarded_from_job_tasks(self):
        cases = [
            ("auto_battle_ct_tie_an_batch", "run_tie_an_jobs"),
            ("auto_battle_ct_regional_ops_batch", "run_regional_ops_jobs"),
            ("auto_battle_gp_action_summary_run_group", "run_group_jobs"),
        ]
        ...
        combat_call_cases = [
            ("auto_battle_ct_tie_an_run_one", "run_combat_after_go_combat_with_difficulty"),
            ("auto_battle_ct_tie_an_run_one", "run_combat_after_go_combat_without_difficulty"),
            ("auto_battle_ct_regional_ops_run_one", "run_combat_after_go_combat"),
            ("auto_battle_gp_action_summary_run_difficulty", "run_combat_after_start_battle"),
        ]
    ...
    def test_battle_ocr_helper_logs_recognized_items(self):
        ...
        with patch.object(battle_dispatch_actions.logger, "debug") as log_debug:
            items = battle_dispatch_actions._recognize_text_items(app, ocr, (10, 20, 300, 200))
    ...
    def test_action_summary_selector_uses_left_drag_for_later_stage(self, _sleep):
        ...
        enter_button = [
            {
                "text": "进入挑战",
                "normalized": battle_dispatch_actions._normalize_text("进入挑战"),
                "center": (948, 606),
                "confidence": 0.99,
            }
        ]
        transition = [
            {
                "text": "开始作战",
                "normalized": battle_dispatch_actions._normalize_text("开始作战"),
                "center": (965, 502),
                "confidence": 0.98,
            }
        ]
        ...
        with (
            ...
            patch.object(battle_dispatch_actions.logger, "info") as log_info,
        ):
    ...
    def test_action_summary_selector_reports_unavailable_stage(self, _sleep):
        title = [
            {
                "text": "特供·救世",
                "normalized": battle_dispatch_actions._normalize_text("特供·救世"),
                "center": (577, 422),
                "confidence": 0.87,
            }
        ]
        unavailable = [
            {
                "text": "周一、日开放",
                "normalized": battle_dispatch_actions._normalize_text("周一、日开放"),
                "center": (590, 606),
                "confidence": 0.98,
            }
        ]
    ...
    def test_action_summary_task_uses_swapped_drag_params(self):
        ...
        steps = self.task_data["auto_battle_gp_action_summary_run_group"]["steps"]
    ...
    def test_action_summary_difficulty_requires_start_battle_business_success(self):
        steps = self.task_data["auto_battle_gp_action_summary_run_difficulty"]["steps"]
    ...
    def test_all_battle_task_drags_hold_before_release(self):
        expected_custom_actions = {
            "resonance.select_ordered_city": 2,
            "resonance.select_threat_level_numeric": 1,
            "resonance.select_action_summary_stage": 1,
        }
