...
from plans.resonance_pc.src.actions.trade_planner_pc_actions import (
...
def test_trade_loop_state_accumulates_rounds_and_total_books():
    async def run():
        ...
        first = await resonance_pc_trade_loop_update(run_key, _round_plan("s1", "A", "B"), state_store=store)
        assert store.data[run_key]["current_city_key"] == ""
        second = await resonance_pc_trade_loop_update(run_key, _round_plan("s2", "B", "A"), state_store=store)
        summary = await resonance_pc_trade_loop_summary(run_key, state_store=store)
        cleanup = await resonance_pc_trade_loop_cleanup(run_key, state_store=store)
...
def test_trade_loop_state_stops_after_final_prefix():
    async def run():
        ...
        summary = await resonance_pc_trade_loop_update(init["run_key"], plan, state_store=store)
...
def test_route_execution_stops_on_fatigue_block_and_records_medicine():
    async def run():
        ...
        init = await resonance_pc_trade_route_execution_init(route, state_store=store)
        run_key = init["run_key"]

        first = await resonance_pc_trade_route_execution_update(
            run_key,
            leg=route[0],
            travel_status="ok",
            state_store=store,
        )
        blocked = await resonance_pc_trade_route_execution_update(
            run_key,
            leg=route[1],
        ...
        summary = await resonance_pc_trade_route_execution_summary(run_key, state_store=store)
...
def test_trade_loop_update_records_blocked_execution_and_stops():
    async def run():
        ...
        summary = await resonance_pc_trade_loop_update(
            init["run_key"],
