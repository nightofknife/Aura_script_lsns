...
def test_combined_strict_nested_contract_keeps_book_mode_and_recovery(order, auto_book):
    task = _load_task("auto_combined_commerce_pc.yaml", "auto_combined_commerce_pc")
    snapshot = {
    ...
    ok, validated = InputValidator(None).validate_inputs_against_meta(task["meta"]["inputs"], {
        ...
        "trade_inputs": {"auto_book": auto_book, "book_budget": 6,
    ...
    assert ok, validated
    assert validated["trade_inputs"]["auto_book"] is auto_book
    assert validated["trade_inputs"]["auto_sparkling_water"] is True
    assert validated["trade_inputs"]["required_end_city_ids"] == ["15"]
