...
from plans.resonance_pc.src.services.resonance_pc_market_data_service import (
...
def _build_service(
    ...
    return ResonancePcMarketDataService(plan_root=plan_root)
...
def test_refresh_failure_fallback_and_no_cache_error(tmp_path: Path):
    ...
    with pytest.raises(ResonancePcMarketDataError) as exc:
...
def test_travel_fatigue_dataset_consistency():
    service = ResonancePcMarketDataService()
