...
def test_profile_defaults_shortcuts_and_disabled_selection_retention(panel):
    ...
    assert "用户信息 5 项" in panel.selection_summary.text()
...
def test_profile_selection_is_persisted_and_empty_only_rejected_when_enabled(panel):
    panel._select_stages(("profile",), profile_sections=("bento_count", "sparkling_water"))
    inputs = panel.collect_inputs()
    assert inputs["profile_sections"] == ["sparkling_water", "bento_count"]
...
def test_version_three_migration_adds_only_original_three_and_runs_once(tmp_path, stages, categories):
    ...
    assert repository.load_player_data_inputs()["profile_sections"] == ["bento_count"]
...
def test_profile_snapshot_partial_values_and_times_preserve_unselected(panel):
    ...
    fresh = {
        "recovery": {"bento_count": {"count": 0}},
        "metadata": {"persisted": True, "refreshed_at": "2026-09-05T01:00:00Z", "section_updated_at": {"profile": "new"}, "profile_section_updated_at": {"bento_count": "bento-count-new"}},
    }
    ...
    assert panel.profile_value_labels["bento_count"].text() == "0 / 12 份"
    assert panel.profile_time_labels["cargo"].text() == "cargo-old"
    assert panel.profile_time_labels["bento_count"].text() == "bento-count-new"
    ...
    assert panel._snapshot["recovery"]["bento_count"] == fresh["recovery"]["bento_count"]


def test_bento_count_partial_refresh_preserves_other_resources(panel):
    old = {
        "recovery": {
        ...
        "metadata": {"profile_section_updated_at": {
            "sparkling_water": "water-old", "bento_count": "bento-old",
        }},
    }
    original = copy.deepcopy(old)
    panel.set_snapshot(old)
    fresh = {
        ...
        "metadata": {"persisted": True, "profile_section_updated_at": {"bento_count": "bento-new"}},
    }
    expected_count = copy.deepcopy(fresh["recovery"]["bento_count"])
    panel.apply_refresh_result(fresh)
    assert old == original
    assert panel.profile_value_labels["bento_count"].text() == "9 / 12 份"
    assert panel.profile_time_labels["bento_count"].text() == "bento-new"
    for key in ("sparkling_water",):
    ...
    fresh["recovery"]["bento_count"]["count"] = 10
    assert panel._snapshot["recovery"]["bento_count"] == expected_count
    ...
    panel.apply_refresh_result({
        "recovery": {"bento_count": {"count": 0}},
        "metadata": {"persisted": True, "profile_section_updated_at": {"bento_count": "bento-empty"}},
    })
    assert panel.profile_value_labels["bento_count"].text() == "0 / 12 份"
    assert panel.profile_time_labels["bento_count"].text() == "bento-empty"
    assert panel.profile_value_labels["sparkling_water"].text() == "剩余免费次数 0 / 6"
...
def test_legacy_profile_time_is_frozen_before_first_partial_refresh(panel):
    ...
    panel.apply_refresh_result({"recovery": {"bento_count": {"count": 2}}, "metadata": {"persisted": True, "section_updated_at": {"profile": "new"}, "profile_section_updated_at": {"bento_count": "new"}}})
    assert panel._snapshot["metadata"]["profile_legacy_updated_at"] == old_time
    assert panel.profile_time_labels["cargo"].text() == expected
    assert panel.profile_time_labels["bento_count"].text() == "new"
...
def test_headless_profile_layout_preview(panel, tmp_path):
    ...
    panel.set_snapshot({"status": {"cargo": {"current": 126, "max": 748}}, "recovery": {"sparkling_water": {"remaining_free_uses": 6, "daily_free_limit": 6}, "bento_count": {"count": 3}}, "metadata": {"profile_section_updated_at": {key: "2026-09-05T01:00:00Z" for key in PLAYER_DATA_PROFILE_SECTION_ORDER}}})
    QApplication.processEvents()
