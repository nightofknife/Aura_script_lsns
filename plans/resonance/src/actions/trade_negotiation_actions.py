"""Template-driven bargaining and raising helpers for Resonance trading."""
...
def _normalize_region(value: Any, *, name: str) -> Tuple[int, int, int, int]:
    if not isinstance(value, (list, tuple)) or len(value) != 4:
        raise ValueError(f"{name} must be [x, y, width, height]")
    region = tuple(int(part) for part in value)
...
def _confirm_cap(
    *,
    ...
    vision: Any,
    cap_template: str,
    cap_region: Tuple[int, int, int, int],
    cap_threshold: float,
...
def _execute_negotiation_to_cap(
    *,
    ...
    cap_region: Any = _CAP_REGION,
    button_region: Any = _BUTTON_REGION,
    cap_threshold: float = _CAP_THRESHOLD,
    button_threshold: float = _BUTTON_THRESHOLD,
) -> Dict[str, Any]:
    ...
    resolved_cap_region = _normalize_region(cap_region, name="cap_region")
    resolved_button_region = _normalize_region(button_region, name="button_region")
    resolved_cap_threshold = float(cap_threshold)
    resolved_button_threshold = float(button_threshold)
    if not 0.0 <= resolved_cap_threshold <= 1.0:
        raise ValueError("cap_threshold must be in [0,1]")
    if not 0.0 <= resolved_button_threshold <= 1.0:
    ...
    while time.monotonic() < deadline:
        last_cap = _confirm_cap(
            ...
            cap_region=resolved_cap_region,
            cap_threshold=resolved_cap_threshold,
            confirmation_interval_sec=cap_confirmation_interval_sec,
        )
        logger.info(
            "resonance negotiation cap_check kind=%s confirmed=%s confidence=%.4f",
            kind,
        ...
        last_button = _wait_for_template_state(
            ...
            region=resolved_button_region,
            threshold=resolved_button_threshold,
            expected_found=True,
        ...
        logger.info(
            "resonance negotiation button_clicked kind=%s confidence=%.4f",
            kind,
        ...
        logger.info(
            "resonance negotiation animation_finished kind=%s button_confidence=%.4f",
            kind,
