"""ResonancePc market data service: fetch, normalize, cache, and query."""
...
class ResonancePcMarketDataError(RuntimeError):
...
@service_info(
    alias="resonance_pc_market_data",
    public=True,
    singleton=True,
    description="Fetches and normalizes ResonancePc market snapshots with local cache.",
)
class ResonancePcMarketDataService:
    ...
    def _extract_assigned_array_expression(chunk_text: str, var_name: str, from_index: int = 0) -> str:
        ...
        start = match.end() - 1
        return ResonancePcMarketDataService._extract_balanced_brackets(chunk_text, start)
