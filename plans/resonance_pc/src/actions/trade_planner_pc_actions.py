"""Actions for ResonancePc trade planner service."""
...
def _require_service(service: Optional[ResonancePcTradePlannerService]) -> ResonancePcTradePlannerService:
    if service is None:
        raise RuntimeError("resonance_pc_trade_planner service is not available.")
    return service
...
def resonance_pc_trade_plan_optimal_route(
    ...
    book_budget: int = 0,
    book_profit_threshold: Any = 15000,
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_plan_next(
    start_city_id: str,
    fatigue_budget: int,
    book_budget: int,
    cargo_capacity: int,
    ...
    return _require_service(resonance_pc_trade_planner).plan_next_step(
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_plan_best_cycle(
    ...
    return _require_service(resonance_pc_trade_planner).plan_best_cycle(
        cargo_capacity=cargo_capacity,
        book_budget=book_budget,
        book_profit_threshold=book_profit_threshold,
        available_city_ids=available_city_ids,
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_plan_cycle_execution(
    fatigue_budget: int,
    current_city_key: Optional[str] = None,
    current_city_id: Optional[str] = None,
    current_city: Optional[str] = None,
    ...
    return _require_service(resonance_pc_trade_planner).plan_cycle_execution(
        current_city_key=current_city_key,
        fatigue_budget=fatigue_budget,
        current_city_id=current_city_id,
        current_city=current_city,
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_plan_next_cycle_execution(
    fatigue_budget: int,
    current_city_key: Optional[str] = None,
    current_city_id: Optional[str] = None,
    current_city: Optional[str] = None,
    ...
    return _require_service(resonance_pc_trade_planner).plan_next_cycle_execution(
        current_city_key=current_city_key,
        fatigue_budget=fatigue_budget,
        current_city_id=current_city_id,
        current_city=current_city,
...
def _loop_state_key() -> str:
    ...
    if not cid or cid == "-":
        cid = uuid.uuid4().hex
    return f"resonance_pc.auto_cycle_trade.{cid}"
...
async def resonance_pc_trade_loop_init(
    current_city: str,
    current_city_key: Optional[str] = None,
...
@action_info(
    name="resonance_pc.trade_route_execution_init",
    public=True,
    read_only=False,
    description="Initialize sequential execution state for one planned trade route.",
)
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_route_execution_init(
...
@action_info(
    name="resonance_pc.trade_route_execution_update",
    public=True,
    read_only=False,
    description="Update sequential route execution state after one trade leg.",
)
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_route_execution_update(
    run_key: str,
...
@action_info(
    name="resonance_pc.trade_route_execution_summary",
    public=True,
    read_only=True,
    description="Return sequential route execution state.",
)
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_route_execution_summary(
    run_key: str,
    state_store: StateStoreService | None = None,
) -> Dict[str, Any]:
    if state_store is None:
...
@action_info(
    name="resonance_pc.trade_route_execution_cleanup",
    public=True,
    read_only=False,
    description="Remove sequential route execution state.",
)
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_route_execution_cleanup(
    run_key: str,
    state_store: StateStoreService | None = None,
) -> Dict[str, Any]:
    if state_store is None:
...
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_loop_update(
    run_key: str,
...
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_loop_summary(
    run_key: str,
    state_store: StateStoreService | None = None,
) -> Dict[str, Any]:
    if state_store is None:
...
@requires_services(state_store="core/state_store")
async def resonance_pc_trade_loop_cleanup(
    run_key: str,
    state_store: StateStoreService | None = None,
) -> Dict[str, Any]:
    if state_store is None:
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_assert_allowed_city(
    city_key: Optional[str] = None,
    city_id: Optional[str] = None,
    resonance_pc_trade_planner: ResonancePcTradePlannerService | None = None,
) -> Dict[str, Any]:
    return _require_service(resonance_pc_trade_planner).assert_allowed_city(
        city_key=city_key,
        city_id=city_id,
    )
...
@requires_services(resonance_pc_trade_planner="resonance_pc_trade_planner")
def resonance_pc_trade_simulate(
    start_city_id: str,
    fatigue_budget: int,
    book_budget: int,
    cargo_capacity: int,
    ...
    return _require_service(resonance_pc_trade_planner).simulate_until_stop(
        start_city_id=start_city_id,
        fatigue_budget=fatigue_budget,
        book_budget=book_budget,
        cargo_capacity=cargo_capacity,
