...
必须从无垠乱斗活动首页启动，真实客户区必须为 1280×720。任务不从城市导航、不自动续接中断的局。GUI 保存输入，只显示阶段、局数、完成数量和错误，不显示逐局运行记录或日志路径。
...
控件和 SELECT 门槛 0.90；角色与装备完整名称均为 0.80，首二名差均为 0.05；装备槽图案 0.88、差 0.10；人物辅助 0.93、差 0.05；宝箱 0.90。始终取全目录最高匹配对象，运行中不自动降低门槛。没有应用半像素重采样实验。
...
同步输入、窗口读取、素材加载和日志 Handler 调用通过 `_run_blocking` 在线程执行；视觉探测复用共享 `poll_until` 的线程路径。取消会禁止尚未开始的排队操作，并等待已开始的操作收尾后再报告停止，避免后台输入或缓存读写残留。超时后返回的识别结果不算成功；这不承诺强制终止已经开始的本机调用。
...
所有运行明细使用既有 Aura logger，写入程序原有的 `aura_session_*.log`，以 `[EternalScuffle]` JSON 标记记录类型、顶层 CID 和局次。候选分数、决策、点击、队伍变化、每局完成及失败时的状态／观察结果／最后目标均记录在这里。本任务不配置新 FileHandler，不创建独立日志目录、JSON 或诊断 PNG；已有历史文件不自动删除。

最终每局结果来自框架已有的循环子任务返回值：`scuffle_checked_round_pc` 校验并转发 `round_result`，主任务将循环结果交给 finish 核对顺序、结果和计数。完成判定不读取日志，不受日志轮转影响，也不在会话里额外积累完整操作历史。
